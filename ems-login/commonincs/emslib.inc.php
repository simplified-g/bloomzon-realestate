...
var $user_id; var $acc_type;var $cathome; var $user ='EMS_usere';var $userboid ='EMS_usueboid';
var $domain = "http://www.emiserve.com/apps/emscms_virginclean/";
var $longpath = "C:/xampp/htdocs/gohome"; 
var $app_mobile; var $app_portal ="";
	...
	public function just_notify($msg,$mode=1,$close=0){
	...
	public function wordify($word,$type=0){
		...
		return $this->connection->real_escape_string(trim(htmlspecialchars($word,ENT_QUOTES)));
	}
		else return $this->connection->real_escape_string(trim(htmlspecialchars($word,ENT_QUOTES)));
	}
	...
	public function id_to_title($id,$table="case_types"){
		$sql = $thistitle = "";
		if($table == "case_types"){
		$field = "title";	
		$sql = "SELECT $field FROM $table WHERE id='$id' LIMIT 1";
		}elseif($table == "fixed_options"){
		$field = "foptions";	
		$sql = "SELECT $field FROM $table WHERE id='$id' LIMIT 1";
		}
		if($sql != ""){
	...
	public function SHA($regno,$pd){
		$pd = SHA1(substr($regno,-3).$pd);	return $pd;
	}
	...
	public function test_this_mobile($mobile){
		...
		foreach($multimobile as $number){
			if(preg_match("/^(234)[7-9?]\d{9}$/", $number) OR preg_match("/^0{1}[7-9?]\d{9}/", $number)){}else{$invalidmobers[] = $number;}
		}
	...
	$exception = array("othernames","religion","thirdchoice","parent_email","parent_address","other_qualifications","issponsor","addi_info","submit_reg","tokenno","submit");
		foreach($_POST as $k => $v){
		if(in_array($k,$exception)){continue;}
		if(is_array($v)){
			foreach($v as $i => $j){
	...
	public function get_age($dob){
		...
		$cd = date("j");
		$age = $cy - $yob;
		if(($cm<$mob) OR ($cm==$mob && $cd<$ddob)){$age-=1;}
		return $age;
		}
		else return 0;
	}
	
	public function start_alpha($name){
	if((int)$name !=0 ){
	...
	//!============paginator =============
		...
		$sql = "SELECT who,didwhat,why,DATE_FORMAT(wen, '%a %D, %b %Y - %h:%m:%s') FROM yemedugo $where ORDER BY wen DESC LIMIT $perPage OFFSET $startAt";
		if(!$result = $this->connection->query($sql)){
		...
		while($row = $result->fetch_assoc()){
		$who[]	= (int)($row['who']);
		$whoname[$k]	= ($this->regno_to_name($who[$k],"customers")=="")?$this->regno_to_name($who[$k],"staff_details"):$this->regno_to_name($who[$k],"customers");
		...
		*/
		...
		if($c<1){
		return $this->my_notifier("Log Is Empty",2);
		}
		else{
		...
		<div class='btn-group'>
		<button class='btn btn-box-tool dropdown-toggle' data-toggle='dropdown'><i class='fa fa-random'></i></button>
	...
	public function paginator_create($thesql,&$total,&$numofPages,&$perPage,&$startAt,&$pageNo,&$prev,&$next,&$sortby='id',&$sortmode=1){
	...
	public function paginator_show($thesql,$total,$numofPages,$perPage,$startAt,$pageNo,$prev,$next,$sortby='id',$sortmode=1){
	echo "  <div class='text-center'>
		<ul class='pagination pagination-lg'>";
		echo "
		<li><a href='?p=1&q=$perPage&all=1&sb=$sortby&sm=$sortmode'>Show All</a></li>
		";
		if($pageNo > 1){
		echo "
		<li>
		  <a href='?p=$prev&q=$perPage&sb=$sortby&sm=$sortmode' aria-label='Previous'>
			<span aria-hidden='true'>&laquo; Prev</span>
		  </a>
		</li>
		<li><a href='?p=1&q=$perPage&sb=$sortby&sm=$sortmode'>First</a></li>
		";	
		}
		...
		elseif($pageNo <  $numofPages){
		echo "<li ><a href='?p=$numofPages&q=$perPage&sb=$sortby&sm=$sortmode'>$numofPages</a></li>
	...
	echo " </ul></div>";
	}/*End of paginator_show Function*/
			
	public function count_this_records($field="",$table="",$where=1){
	$result="";
	if($where==1){
	$sql = "SELECT COUNT(*) FROM $table WHERE 1";
	$result = $this->connection->query($sql);
	...
	public function get_http_referer($ct=0,$mode=0){
		$ref = isset($_SERVER['HTTP_REFERER'])?$this->printify(basename($_SERVER['HTTP_REFERER']),1):"javascript:history.back(1)";
		if($mode==1){
		$ct = isset($_REQUEST['ct'])?(int)$_REQUEST['ct']:$ct;
	...
	public function att_manager($newfilename,$targetpath="",$ftype=0,$errorpage=1,$successpage=""){
		//MOST RECENT AND MORE PREFERED TO UPLOAD_MANAGER
		$targetpath = $targetpath == ""?"attachments/":$targetpath;
		/*
		insert this unto the form		  
		<input type="hidden" name="uploadname" value="attachment">
		*/
		$uploadname = isset($_REQUEST['uploadname'])?$this->wordify($_REQUEST['uploadname'],1):"attachment";
		$ext = pathinfo($_FILES["$uploadname"]["name"], PATHINFO_EXTENSION);
		$ext = strtolower($ext);
		$att_reply = ""; $att_error = 0;
		if($ftype==1){
		$allowedExts = array("jpg","jpeg","png","doc","docx","pdf","xls","xlsx","ppt","pptx","xps","xpsx");
		}else{
		$allowedExts = array("jpg","jpeg","png");
		}
		$target = $targetpath . basename( $_FILES["$uploadname"]["name"]) ;
		$filename = basename( $_FILES["$uploadname"]["name"]); 
		$file_size = $_FILES["$uploadname"]["size"];
		$file_type = strtolower($_FILES["$uploadname"]["type"]);
		if(!in_array($ext,$allowedExts)){
		$att_error = 2;
		$this->goto_notify($errorpage,"$file_type is Not Acceptable");
		}

		if($_FILES["$uploadname"]["size"] > 5124000) {$att_error = 3;
		$this->goto_notify($errorpage,'File is Too Large, Try Reducing or Use JPG');
		} 
		...
		unlink($newname);
		//$att_error = 4;
		//$this->goto_notify($errorpage,'Sorry There was An Error With The Attachment, Please Try re-attaching it');
		}
			if(!rename($target,$newname)){$att_error = 5;
			$this->goto_notify($errorpage,'Could Not Attach File Try Later');
			}
			else{
				if($att_error == 0){
				$att_reply =  $newfilename . ".".$ext;
				if($successpage==""){return $att_reply;}
				else{$this->goto_notify($successpage,'Upload Successful');}
				}
			}
		}
	}
	
	public function make_thumb($uploaded_time,$x=230,$y=100,$path="",$replace=0){
	$targetpath = ($path=="")?"../ascends/":$path; 
	 ...
	 $thumbname = "th_".$orig;
	 if(!file_exists($thumbpath)){$imgt($dst, $thumbpath);}
	}else{
	...
	/* Type1 = Passport, Type2=Certificates */
		$file = ($type==1)?"../ascends/"+$file:"../ascended_certs/"+$file;
		if(file_exists($file)){
		unlink($file);
	...
	public function pagename(){
		return htmlspecialchars(basename($_SERVER['PHP_SELF']),ENT_QUOTES);
	}
	
	public function base($outp = 1){
		$base = 'http://' . $_SERVER['HTTP_HOST'] ."/". $this->basepath;
		if($outp==1){echo $base;}else return $base;
	}
	
	public function tagify($tt,$class=''){
		if($tt=="")return "";//"<span class='fhint'>No Tags</span>";
		$tags = explode(", ",$tt);
		$tagurl = "";
		$baseurl = $this->base(0);
		foreach ($tags as $t){
		$tagurl	.= "<a href='$baseurl/search/$t' class='$class'>$t</a> ";
		}
		return $tagurl;
	}

	public function remove_chars($char,$type=0){
	if($type==0){return preg_replace("/[^A-Za-z0-9\.\-\ ]/", "", $char);}
		else {return preg_replace("/[^A-Za-z0-9\.\-]/", "", $char);}
	}
