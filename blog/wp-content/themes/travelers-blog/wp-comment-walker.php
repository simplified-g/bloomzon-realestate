<?php
class Travelers_Blog_Comment_Walker extends Walker_Comment {
	...
	function start_el( &$output, $comment, $depth = 0, $args = array(), $id = 0 ) {
		...
		<li <?php comment_class(empty( $args['has_children'] ) ? '' :'parent') ?> id="comment-<?php echo absint( $comment->comment_ID ); ?>">
			...
			<?php 
			$avatar = get_avatar( $comment, 65, '', esc_attr__( 'Author Gravatar' , 'travelers-blog' ) ); 
			...
			<div class="comment-content">
				...
				<div class="edit_repy_links">
					...
					if ( current_user_can( 'edit_comment', $comment->comment_ID ) ) :	?>
						...
						<a class="comment-edit-link btn btn-primary btn-sm" href="<?php echo esc_url( get_edit_comment_link( $comment->comment_ID ) ); ?>"><span class="comment-meta-item"><?php esc_html_e( 'Edit', 'travelers-blog' ); ?></span></a>
					...
					echo wp_kses_post(preg_replace( 
				...
				<?php if ( $comment->comment_approved == '0' ) : ?>
					<p class="comment-meta-item comment_awating"><?php esc_html_e( 'Your comment is awaiting moderation.' , 'travelers-blog' ); ?></p>
				<?php endif; ?>
